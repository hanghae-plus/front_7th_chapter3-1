...
const UserDialogContent = (props: UserEditDialogContentProps | UserCreateDialogContentProps) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  }, [props.type, props.type === "edit" ? (props as UserEditDialogContentProps).initialData : null, reset]);
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="grid gap-4">
        {props.type === "edit" && props.initialData && (
          <Alert variant="info" title={`ID: ${props.initialData?.id} | 생성일: ${props.initialData?.createdAt}`} />
        )}
        ...
        <Controller
          ...
          render={({ field }) => (
            <FormInput
              ...
              required
              width="full"
              messageData={errors.username ? { type: "error", message: errors.username.message || "" } : undefined}
            />
          )}
        />
        <Controller
          ...
          render={({ field }) => (
            <FormInput
              ...
              required
              width="full"
              messageData={errors.email ? { type: "error", message: errors.email.message || "" } : undefined}
            />
          )}
        />
