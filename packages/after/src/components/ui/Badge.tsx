	...
	'inline-flex items-center justify-center border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0 [&>svg]:size-3 gap-1 [&>svg]:pointer-events-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive transition-[color,box-shadow] overflow-hidden',
	{
		variants: {
			...
			type: {
				...
				danger:
					'border-transparent bg-error text-error-foreground [a&]:hover:bg-error-hover focus-visible:ring-error/20 dark:focus-visible:ring-error/40 dark:bg-error/60',
				info: 'border-transparent bg-info text-info-foreground [a&]:hover:bg-info-hover focus-visible:ring-info/20 dark:focus-visible:ring-info/40 dark:bg-info/60',
			},
		},
		defaultVariants: {
			type: 'primary',
...
function Badge({
	...
	return (
		<Comp
			data-slot='badge'
			className={cn(badgeVariants({ type, pill }), className)}
			{...props}
		/>
	);
}
